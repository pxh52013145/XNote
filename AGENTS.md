...
## 日志规则（新增）
- 仓库根目录维护 `logs/` 文件夹，用于记录阶段性计划与执行日志。
- 日志文件命名统一使用 **`YYYY-MM-DD_内容.md`**（日期 + 内容），例如：`2026-02-06_infra-bootstrap.md`。
- 进行中任务应先在 `logs/` 新建或续写对应日志文件，至少包含：阶段目标、实施步骤、当前状态、下一步。
- 每次完成一个可验证里程碑（例如模块骨架、核心接口接入、回归通过）后，及时补充日志条目，保证可追踪、可回滚。
