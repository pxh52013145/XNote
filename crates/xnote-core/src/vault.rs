use crate::paths::{join_inside, normalize_folder_rel_path, normalize_vault_rel_path, to_posix_path};
...
impl Vault {
  ...
  pub fn save_folder_order(&self, folder: &str, ordered_paths: &[String]) -> Result<()> {
    ...
    std::fs::write(&order_path, content).with_context(|| format!("write order file: {:?}", order_path))?;
    Ok(())
  }
}
