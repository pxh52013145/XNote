use gpui::{
  Animation, AnimationExt as _, App, Application, AssetSource, AvailableSpace, Bounds, ClickEvent,
  ClipboardItem, Context, CursorStyle,
  DragMoveEvent, Element, ElementId, ElementInputHandler, Entity, EntityInputHandler, FocusHandle,
  FontWeight, GlobalElementId, KeyDownEvent, LayoutId, MouseButton, MouseDownEvent, MouseMoveEvent,
  ...
  WindowOptions, div, ease_in_out, fill, point, prelude::*, px, radians, relative, rgb, rgba, size,
  svg, uniform_list,
};
use std::borrow::Cow;
...
#[derive(Clone, Debug)]
enum ExplorerRow {
  Vault { root_name: String, expanded: bool },
...
#[derive(Clone, Debug)]
enum SearchRow {
  File { path: String, match_count: usize },
  Match { path: String, line: usize, preview: String },
}

const PALETTE_COMMANDS: &[PaletteCommandSpec] = &[
  PaletteCommandSpec {
    id: PaletteCommandId::OpenVault,
    label: "Open vault",
    detail: "Choose a folder as a vault",
    shortcut: "Ctrl+O",
    icon: ICON_FOLDER_OPEN,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::QuickOpen,
    label: "Quick Open",
    detail: "Search and open a note by path",
    shortcut: "Ctrl+P",
    icon: ICON_SEARCH,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::CommandPalette,
    label: "Command palette",
    detail: "Search and run commands",
    shortcut: "Ctrl+K",
    icon: ICON_COMMAND,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::Settings,
    label: "Settings",
    detail: "Preferences, hotkeys, and more",
    shortcut: "Ctrl+,",
    icon: ICON_SETTINGS,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::ReloadVault,
    label: "Reload vault",
    detail: "Rescan notes and apply folder order",
    shortcut: "Ctrl+R",
  ...
  PaletteCommandSpec {
    ...
    label: "New note",
    detail: "Create a note in the selected folder",
    shortcut: "Ctrl+N",
    icon: ICON_FILE_PLUS,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::SaveFile,
    label: "Save file",
    detail: "Write the current note to disk",
    shortcut: "Ctrl+S",
    icon: ICON_DOWNLOAD,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::ToggleSplit,
    label: "Toggle split",
    detail: "Split the editor view",
    shortcut: "Ctrl+\\",
    icon: ICON_COLUMNS_2,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::FocusExplorer,
    label: "Explorer",
    detail: "Show Explorer panel",
    shortcut: "Alt+1",
    icon: ICON_FOLDER,
  },
  PaletteCommandSpec {
    id: PaletteCommandId::FocusSearch,
    label: "Search",
    detail: "Show Search panel",
    shortcut: "Alt+2",
    icon: ICON_SEARCH,
  },
];
...
impl XnoteWindow {
  fn new(cx: &mut Context<Self>) -> Self {
    let mut this = Self {
      ...
      settings_language: SettingsLanguage::English,
  ...
  fn cursor_line_col(&self) -> (usize, usize) {
    ...
    let cursor = self.editor_cursor_offset().min(self.open_note_content.len());
      ...
      async move {
        ...
        this
          .update(&mut cx, |this, cx| match result {
            ...
            Err(err) => {
              this.status = SharedString::from(format!("Create folder failed: {err}"));
              cx.notify();
            }
          })
          .ok();
  ...
  fn filtered_palette_command_indices(&self) -> Vec<usize> {
    ...
    PALETTE_COMMANDS
      ...
      .filter_map(|(ix, cmd)| {
        ...
        let detail = cmd.detail.to_lowercase();
  ...
  fn execute_palette_command(&mut self, id: PaletteCommandId, cx: &mut Context<Self>) {
    match id {
      PaletteCommandId::OpenVault => {
        self.close_palette(cx);
        self.open_vault_prompt(cx);
      }
      PaletteCommandId::QuickOpen => self.open_palette(PaletteMode::QuickOpen, cx),
      PaletteCommandId::CommandPalette => self.open_palette(PaletteMode::Commands, cx),
      PaletteCommandId::Settings => {
        self.close_palette(cx);
        self.settings_open = true;
        self.settings_language_menu_open = false;
        cx.notify();
      }
      PaletteCommandId::ReloadVault => {
        self.close_palette(cx);
      ...
      PaletteCommandId::SaveFile => {
        self.close_palette(cx);
        self.force_save_note(cx);
      }
      PaletteCommandId::ToggleSplit => {
        self.close_palette(cx);
        self.split_editor = !self.split_editor;
        cx.notify();
      }
      PaletteCommandId::FocusExplorer => {
        self.close_palette(cx);
        self.panel_mode = PanelMode::Explorer;
        cx.notify();
      }
      PaletteCommandId::FocusSearch => {
        self.close_palette(cx);
  ...
  fn effective_sidebar_layout(&self, window_width: Pixels) -> (SidebarState, SidebarState) {
    ...
    let required_width = |panel: SidebarState, workspace: SidebarState| -> Pixels {
      ...
      let splitters = (panel != SidebarState::Hidden) as usize + (workspace != SidebarState::Hidden) as usize;
  ...
  fn set_panel_shell_collapsed(&mut self, collapsed: bool, cx: &mut Context<Self>) {
    if collapsed {
      if !self.panel_shell_collapsed {
        ...
        self.panel_shell_tab_toggle_anim_nonce = self.panel_shell_tab_toggle_anim_nonce.wrapping_add(1);
        cx.notify();
      }
  ...
  fn set_workspace_collapsed(&mut self, collapsed: bool, cx: &mut Context<Self>) {
    if collapsed {
      if !self.workspace_collapsed {
        ...
        self.workspace_tab_toggle_anim_nonce = self.workspace_tab_toggle_anim_nonce.wrapping_add(1);
        cx.notify();
      }
  ...
  fn begin_splitter_drag(&mut self, kind: SplitterKind, event: &MouseDownEvent, cx: &mut Context<Self>) {
    ...
    match drag.kind {
      SplitterKind::PanelShell => {
        ...
        let right = editor_min_w
          + splitter_w
          + if workspace_present { workspace_w + splitter_w } else { px(0.) };
        let reserve = rail_w + right;
        let max_w = if window_w > reserve { window_w - reserve } else { panel_min_w };
      ...
      SplitterKind::Workspace => {
        ...
        let left = rail_w + if panel_shell_present { panel_shell_w + splitter_w } else { px(0.) };
        let reserve = left + splitter_w + editor_min_w;
        let max_w = if window_w > reserve { window_w - reserve } else { workspace_min_w };
        let next = (drag.start_width + delta_x).clamp(workspace_min_w, max_w.max(workspace_min_w));
  ...
  fn on_vault_prompt_key(&mut self, ev: &KeyDownEvent, cx: &mut Context<Self>) {
    ...
    match key.as_str() {
      ...
      "enter" | "return" => {
        ...
        if value.is_empty() {
          self.vault_prompt_error = Some(SharedString::from("Enter a path to a vault folder."));
          cx.notify();
          return;
        }
        ...
        if !path.is_dir() {
          self.vault_prompt_error = Some(SharedString::from("Vault path is not a folder."));
          cx.notify();
          return;
        }
  ...
  fn palette_overlay(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
    let (title, placeholder, group_title) = match self.palette_mode {
      PaletteMode::Commands => ("COMMAND PALETTE", "Type a command…", "NAVIGATION"),
      PaletteMode::QuickOpen => ("QUICK OPEN", "Type to search…", "FILES"),
    };
          ...
          PaletteMode::Commands => {
              ...
              return range
                .map(|ix| {
                  div()
                    ...
                    .child(if ix == 0 { "No commands" } else { "" })
                })
                .collect::<Vec<_>>();
            }
            ...
            range
              .map(|ix| {
                ...
                div()
                  .id(ElementId::Name(SharedString::from(format!("palette.cmd:{cmd_ix}"))))
                  .h(px(44.))
                  .w_full()
                  .px(px(10.))
                  .flex()
                  ...
                  .when(selected, |this| this.border_1().border_color(rgb(0xb6edf5)))
                  .when(selected, |this| {
                    this.child(
                      div()
                        .absolute()
            ...
            range
              .map(|ix| {
                let Some(note_ix) = this.palette_results.get(ix).copied() else {
                  return div()
                    .id(ElementId::named_usize("palette.quick_open.missing", ix))
                    .h(px(44.))
                    .px_3()
                    .child("");
                };
                let Some(path) = this.explorer_all_note_paths.get(note_ix).cloned() else {
                  return div()
                    .id(ElementId::named_usize("palette.quick_open.missing", ix))
                    .h(px(44.))
                    .px_3()
                ...
                div()
                  .id(ElementId::Name(SharedString::from(format!("palette.note:{path}"))))
                  .h(px(44.))
                  .w_full()
                  .px(px(10.))
                  .flex()
    ...
    let palette_box = div()
      ...
      .child(
        div()
          ...
          .child(div().id("palette.list").flex_1().overflow_y_scroll().child(list)),
      );
  ...
  fn settings_overlay(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
    let nav_item = |id: &'static str,
      ...
      div()
        ...
        .overflow_hidden()
        .cursor_pointer()
        .bg(if active { rgb(0xe6f7fa) } else { rgba(0x00000000) })
        ...
        .child(ui_icon(
          icon,
          16.,
          if active { 0x0b4b57 } else { 0x6b7280 },
        ))
        .child(
          div()
            .font_family("Inter")
    ...
    let page_title = match self.settings_section {
      ...
      SettingsSection::FilesLinks => "Files & Links",
      SettingsSection::Hotkeys => "Hotkeys",
      SettingsSection::Advanced => "Advanced",
    };

    let appearance_content = {
      let theme_button = |id: &'static str, label: &'static str, theme: SettingsTheme| {
      ...
      let accent_chip = |id: &'static str, label: &'static str, accent: SettingsAccent| {
      ...
      let language_menu = div()
        ...
        .child(
          div()
            ...
            .hover(|this| this.bg(rgb(0xe6f7fa)))
            .on_click(cx.listener(|this, _ev: &ClickEvent, _window, cx| {
              this.settings_language = SettingsLanguage::English;
              this.settings_language_menu_open = false;
              cx.notify();
            }))
            .child(
      ...
      let language_section = div()
        ...
        .child(
          div()
            ...
            .font_weight(FontWeight(650.))
            .text_color(rgb(0x6b7280))
            .child("Change the UI language."),
        )
        .child(language_select)
        .children(self.settings_language_menu_open.then_some(
          div().mt_1().child(language_menu),
        ));
              ...
              div()
                ...
                .child(
                  div()
                    ...
                    .font_weight(FontWeight(650.))
                    .text_color(rgb(0x6b7280))
                    .child("Used for buttons, links and highlights."),
                )
                .child(accent_row),
            ),
        )
    ...
    let page_content = match self.settings_section {
      ...
      SettingsSection::About => placeholder("About page is not implemented yet."),
      SettingsSection::Editor => placeholder("Editor settings are not implemented yet."),
      SettingsSection::FilesLinks => placeholder("Files & Links settings are not implemented yet."),
      SettingsSection::Hotkeys => placeholder("Hotkeys settings are not implemented yet."),
      SettingsSection::Advanced => placeholder("Advanced settings are not implemented yet."),
    };
        ...
        div()
          ...
          .child(
            div()
              ...
              .flex_col()
              .gap(px(6.))
              .child(nav_item("settings.nav.about", ICON_USER, "About", SettingsSection::About))
              .child(nav_item(
        ...
        let search_rows: Vec<SearchRow> = cx
          ...
          .spawn(async move {
            ...
            for path in note_paths.iter() {
              if files_with_matches >= MAX_FILES_WITH_MATCHES || match_rows >= MAX_MATCH_ROWS {
                break;
              }
  ...
  fn on_palette_key(&mut self, ev: &KeyDownEvent, cx: &mut Context<Self>) {
    ...
    if ctrl {
      match key.as_str() {
        ...
        "o" => {
          self.execute_palette_command(PaletteCommandId::OpenVault, cx);
          return;
        }
        "r" => {
          self.execute_palette_command(PaletteCommandId::ReloadVault, cx);
          return;
        }
        "," => {
          self.execute_palette_command(PaletteCommandId::Settings, cx);
          return;
        }
        "n" => {
          self.execute_palette_command(PaletteCommandId::NewNote, cx);
          return;
        }
        "s" => {
          self.execute_palette_command(PaletteCommandId::SaveFile, cx);
          return;
        }
        "\\" => {
          self.execute_palette_command(PaletteCommandId::ToggleSplit, cx);
          return;
        }
        _ => {}
      }
    ...
    match key.as_str() {
      ...
      "enter" | "return" => {
        match self.palette_mode {
          ...
          PaletteMode::QuickOpen => {
            let Some(note_ix) = self.palette_results.get(self.palette_selected).copied() else {
              return;
            };
  ...
  fn rebuild_explorer_rows(&mut self) {
    ...
    match &self.vault_state {
      VaultState::NotConfigured => rows.push(ExplorerRow::Hint {
        text: SharedString::from("Open a vault (Ctrl+O)"),
      }),
      VaultState::Error { .. } => rows.push(ExplorerRow::Hint {
        text: SharedString::from("Vault error (Ctrl+O)"),
      }),
      _ => {}
    }
  ...
  fn append_folder_contents(&self, folder: &str, depth: usize, rows: &mut Vec<ExplorerRow>) {
    let children = self
      ...
      .map(|v| v.as_slice())
      .unwrap_or(&[]);
    let notes = self.folder_notes.get(folder).map(|v| v.as_slice()).unwrap_or(&[]);

    let total = children.len() + notes.len();
    for ix in 0..total {
      if ix < children.len() {
        let child = &children[ix];
        let name = child.rsplit('/').next().unwrap_or(child.as_str()).to_string();
          ...
          .update(&mut cx, |this, cx| {
            ...
            match read_result {
              Ok(content) => {
                ...
                if let Some((pending_path, pending_line)) = this.pending_open_note_cursor.take() {
                  if pending_path == note_path {
                    let offset = byte_offset_for_line(&this.open_note_content, pending_line);
                    this.editor_selected_range = offset..offset;
                    this.editor_selection_reversed = false;
                    this.editor_preferred_x = None;
                  } else {
                    this.pending_open_note_cursor = Some((pending_path, pending_line));
                  }
                }
  ...
  fn editor_copy(&mut self, cx: &mut Context<Self>) {
    ...
    let Some(text) = self.open_note_content.get(self.editor_selected_range.clone()) else {
      return;
    };
  ...
  fn editor_replace_selection(&mut self, new_text: &str, cx: &mut Context<Self>) {
    ...
    self.open_note_content =
      self.open_note_content[0..range.start].to_owned() + new_text + &self.open_note_content[range.end..];
...
impl NoteEditorLayout {
    ...
    window.request_measured_layout(style, {
      ...
      move |known_dimensions, available_space, window, cx| {
        ...
        if let Some(inner) = element_state.0.borrow().as_ref() {
          if inner.size.is_some() && (wrap_width.is_none() || wrap_width == inner.wrap_width) {
        ...
        for w in boundaries.windows(2) {
          ...
          let in_selection = !selection.is_empty() && start >= selection.start && end <= selection.end;
        ...
        let lines = match window.text_system().shape_text(text, font_size, &runs, wrap_width, None) {
  ...
  fn bounds_for_byte_range(&self, range: Range<usize>) -> Option<Bounds<Pixels>> {
    ...
    let start_pos = self.position_for_index(range.start).unwrap_or(bounds.origin);
    ...
    Some(Bounds::from_corners(point(left, top), point(right + px(2.0), bottom)))
  }
}
...
impl Element for NoteEditorElement {
  ...
  fn paint(
    ...
    let (selected_range, selection_reversed) = {
      ...
      (view.editor_selected_range.clone(), view.editor_selection_reversed)
    };
    ...
    if focus_handle.is_focused(window) && selected_range.is_empty() {
      ...
      if let Some(cursor_pos) = layout.position_for_index(cursor) {
        let cursor_quad: PaintQuad = fill(
          Bounds::new(point(cursor_pos.x, cursor_pos.y), size(px(2.0), layout.line_height().unwrap_or(px(16.0)))),
          rgb(0x2563eb),
        );
...
impl EntityInputHandler for XnoteWindow {
  fn text_for_range(
    ...
    let range = range.start.min(self.open_note_content.len())..range.end.min(self.open_note_content.len());
  ...
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let rail_button = |id: &'static str,
      ...
      div()
        ...
        .flex()
        .items_center()
        .bg(if active { rgb(0xe1e5ea) } else { rgba(0x00000000) })
        .hover(|this| this.bg(rgb(0xe1e5ea)))
        .cursor_pointer()
        .on_click(cx.listener(move |this, _ev: &ClickEvent, _window, cx| {
          on_click(this, cx);
        }))
        .child(div().w(px(3.)).h_full().bg(if active { rgb(0xb6edf5) } else { rgba(0x00000000) }))
        .child(div().w_full().h_full().flex().items_center().justify_center().child(ui_icon(icon, 20., icon_color)))
    };
    ...
    let rail_bottom = div().flex().flex_col().justify_end().h(px(64.)).child(rail_button(
      "rail.settings",
      ICON_SETTINGS,
      0x6b7280,
      false,
    ...
    let available_for_expanded = if window_w > reserve { window_w - reserve } else { px(0.) };
    ...
    match (panel_shell_state, workspace_state) {
      (SidebarState::Expanded, SidebarState::Expanded) => {
        ...
        let max_workspace = (available_for_expanded - panel_shell_expanded_w).max(workspace_min_w);
    ...
    if panel_shell_state == SidebarState::Expanded && self.panel_shell_width != panel_shell_width {
    ...
    let explorer_panel = div()
      ...
      .child(
        div()
          ...
          .child(
            ui_icon(ICON_FUNNEL, 14., 0x6b7280),
          )
          .child(
            div()
              .font_family("IBM Plex Mono")
              .text_size(px(11.))
              .font_weight(FontWeight(if self.is_filtering() { 900. } else { 650. }))
              ...
              cx.processor(|this, range: std::ops::Range<usize>, _window, cx| {
                  ...
                  return range
                    .map(|ix| {
                      let Some(note_ix) = matches.get(ix) else {
                        return div()
                          .id(ElementId::named_usize("explorer.filtered.missing", ix))
                          .px_3()
                          .py_2()
                          .child("");
                      };
                      let Some(path) = this.explorer_all_note_paths.get(*note_ix) else {
                        return div()
                          .id(ElementId::named_usize("explorer.filtered.missing", ix))
                          .px_3()
                          .py_2()
                ...
                range
                  .map(|ix| match rows.get(ix) {
                    Some(ExplorerRow::Vault { root_name, expanded }) => {
                      if is_filtering {
                        return div()
                          .id(ElementId::named_usize("explorer.vault.hidden", ix))
                          .px_3()
                          .py_2()
                          .child("");
                      }

                      let chevron = if *expanded { ICON_CHEVRON_DOWN } else { ICON_CHEVRON_RIGHT };
                    ...
                    Some(ExplorerRow::Folder {
                      ...
                      if is_filtering {
                        return div()
                          .id(ElementId::named_usize("explorer.folder.hidden", ix))
                          .px_3()
                          .py_2()
                          .child("");
                      }
                      ...
                      let indent_el = tree_indent_guides(*depth, line_mask);
                    ...
                    Some(ExplorerRow::Note {
                      ...
                      let is_selected = this.selected_note.as_deref() == Some(path.as_str());
                      let is_drag_target = this
                        .drag_over
                        .as_ref()
                        .is_some_and(|d| d.folder == *folder && d.target_path == *path);
                      ...
                      let selected_path = path.clone();
                      let display_name = if is_filtering { path.clone() } else { file_name.clone() };

                      let line_mask = depth_line_mask(*depth);
                      let indent_el = tree_indent_guides((*depth).max(1), line_mask);

                      let icon_color = if is_selected { 0x0b4b57 } else { 0x6b7280 };
                      let text_color = if is_selected { 0x0b4b57 } else { 0x111827 };
                      let text_weight = if is_selected { 900. } else { 750. };
    ...
    let mode_bar = div()
      ...
      .items_center()
      .gap(px(6.))
      .child(workspace_mode_button("workspace.mode.open", ICON_FILE_TEXT, WorkspaceMode::OpenEditors))
      ...
      if self.open_editors.is_empty() {
        ...
        for path in &self.open_editors {
          ...
          let close_button = div()
            .id(ElementId::Name(SharedString::from(format!("open.close:{close_path}"))))
            .h(px(28.))
            .w(px(24.))
            .flex()
            .items_center()
          ...
          open_list = open_list.child(
            div()
              ...
              .items_center()
              .gap_2()
              .bg(if is_active { rgb(0xe1e5ea) } else { rgba(0x00000000) })
              .cursor_pointer()
              .when(!is_active, |this| this.hover(|this| this.bg(rgb(0xe1e5ea))))
              .on_click(cx.listener(move |this, _ev: &ClickEvent, _window, cx| {
                this.open_note(path.clone(), cx);
              }))
              .child(div().w(px(3.)).h_full().bg(if is_active { rgb(0xb6edf5) } else { rgba(0x00000000) }))
              .child(ui_icon(ICON_FILE_TEXT, 14., icon_color))
              .child(
                div()
                  .flex_1()
    ...
    let sidebar_toggle_anim = Animation::new(Duration::from_millis(160)).with_easing(ease_in_out);
    let tab_action_animated = |id: &'static str,
                              icon: &'static str,
                              animation_id: ElementId,
                              exiting: bool,
                              on_click: fn(&mut XnoteWindow, &mut Context<XnoteWindow>)| {
      tab_action(id, icon, on_click)
        .relative()
        .with_animation(
          animation_id,
    ...
    let show_panel_shell_toggle = self.panel_shell_collapsed || self.panel_shell_tab_toggle_exiting;
    ...
    for path in &self.open_editors {
      ...
      let close_button = div()
        .id(ElementId::Name(SharedString::from(format!("tab.close:{close_path}"))))
        .h(px(28.))
        .w(px(20.))
        .flex()
        .items_center()
      ...
      tabs_bar = tabs_bar.child(
        div()
          ...
          .max_w(px(220.))
          .overflow_hidden()
          .bg(if is_active { rgb(0xffffff) } else { rgb(0xeef0f2) })
          .cursor_pointer()
          .when(!is_active, |this| this.hover(|this| this.bg(rgb(0xe1e5ea))))
          .on_click(cx.listener(move |this, _ev: &ClickEvent, _window, cx| {
            this.open_note(path.clone(), cx);
    ...
    let editor_pane = |id: &'static str, interactive: bool| {
      ...
      if self.open_note_path.is_some() && !self.open_note_loading {
        ...
        pane.child(NoteEditorElement { view: cx.entity() }).into_any_element()
      } else {
        pane
          .font_family("IBM Plex Mono")
          .text_size(px(11.))
    ...
    let titlebar_window_button = |id: &'static str,
                                  icon: &'static str,
      ...
      div()
        ...
        .cursor_pointer()
        .hover(|this| this.bg(rgb(0xe1e5ea)))
        .on_click(move |_ev: &ClickEvent, window: &mut Window, _cx: &mut App| {
          on_click(window);
        })
        .child(ui_icon(icon, 16., 0x6b7280))
    };
    ...
    let titlebar_window_controls = div()
      ...
      .items_center()
      .justify_end()
      .child(titlebar_window_button("titlebar.min", ICON_MINUS, |window| {
        window.minimize_window();
      }))
      .child(titlebar_window_button("titlebar.max", ICON_SQUARE, |window| {
        window.zoom_window();
      }))
    ...
    let menu_bar = div()
      ...
      .items_center()
      .child(titlebar_left)
      .child(div().w(px(520.)).flex().items_center().justify_center().child(titlebar_command))
      .child(
        div()
          .flex_1()
          .h_full()
            ...
            div()
              ...
              .child(
                div()
                  ...
                  .font_weight(FontWeight(750.))
                  .text_color(rgb(0x374151))
                  .child(SharedString::from(format!("Workspace: {workspace_name}"))),
              )
              .child(ui_icon(ICON_CHEVRON_DOWN, 16., 0x6b7280)),
          )
          .child(div().w(px(1.)).h(px(14.)).bg(rgb(0xc8cdd5)))
    ...
    if self.splitter_drag.is_some() {
      root = root.child(
        div()
          ...
          .on_mouse_up(MouseButton::Left, cx.listener(Self::on_splitter_drag_mouse_up))
          .on_mouse_up_out(MouseButton::Left, cx.listener(Self::on_splitter_drag_mouse_up)),
      );
    }
...
fn build_explorer_index(vault: &Vault, entries: &[NoteEntry]) -> anyhow::Result<ExplorerIndex> {
  ...
  for folder in folder_keys {
    ...
    loop {
      ...
      child_sets.entry(parent.clone()).or_default().insert(full.clone());
      if parent.is_empty() {
        break;
      }
      full = parent;
